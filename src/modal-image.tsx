...
export function ModalImage({
    ...
    const isOpenRefEffect = () => {
        ...
        if(isOpenRef.current) {
            try {
                ...
                setImageFunction(menuHookRef.current.toggleStill);
    ...
    const display = () => { // This has to be all CSS
        ...
        if(isOpenRef.current) {
            ...
            // 3.2. change background color if given
            setBackgroundColor();
    ...
    const setBackgroundColor = () => {
        ...
        typeof backgroundColor === 'string' && (bg = backgroundColor);
        typeof bgColor === 'string' && (bg = bgColor);
        modalRef.current && (modalRef.current.style.backgroundColor = bg);
    };
    const viewportIs = () => {
    ...
    const resizeImage = () => {
        imageRef.current && (
            ...
            imageRef.current.style.setProperty('min-width', 'auto'),
            imageRef.current.style.setProperty('min-height', 'auto'),
            imageRef.current.style.setProperty('max-width', 'none'),
            imageRef.current.style.setProperty('max-height', 'none')
        );
        const imageWidth = imageComputedSize.current?.width ?? imageRef.current?.width ?? 0;
        const imageHeight = imageComputedSize.current?.height ?? imageRef.current?.height ?? 0;
    ...
    const alterElementClass = (element: RefObject<HTMLDivElement | HTMLImageElement | null> | HTMLElement, action?: 'add' | 'addClass' | 'removeClass' | 'replaceClass', classString?: string | '') => {
        ...
        if(alteringElement) {
            switch(action) {
                case 'add': alteringElement.className += classString;
                    break;
                case 'addClass': alteringElement.classList.add(classString ?? '');
                    break;
                case 'removeClass': alteringElement.classList.remove(classString ?? '');
                    break;
                default: alteringElement.className = classString ?? '';
            };
        }
    };
    ...
    const setImageFunction = (func: () => void) => {
        if(onClickFunction) {
            imageRef.current && (imageRef.current.onclick = onClickFunction);
        } else {
            if(!ismap && !useMap && !noModal) {
                imageRef.current && (imageRef.current.onclick = func);
            }
        }
    }
    ...
    return (
        <>
        <div>
            <div ref={placeHolderRef} style={{ width: imageComputedSize.current?.width, height: imageComputedSize.current?.height}} className='hide'></div>
            <div ref={modalRef}>
                <ImageMemo 
                    ref={imageRef}
                    src={src}
