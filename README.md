...
A lightweight image component for React (drop-in replacement for `<img />`) with functionality to display image in a modal with CSS and opens without re-rendering the original image. Simply modal-image is high level standard HTML image tag on top of jsx `<img />` component. That means modal-image accepts all jsx properties and HTML image attributes. So, anyone can start using it hassle free.

Found inspiration to make modal-image from [react-modal-image](https://github.com/aautio/react-modal-image) an implementation of Lightbox (nowadays [Lightbox 2](https://github.com/lokesh/lightbox2)) which is a very popular, good way to show image on screen without distraction and it's nice to have that functionality for all images on web as native functionality of image content.
...
- 😱 No side effect - carefully planned and designed component and CSS styles
...
> Image alt(alternate) text will be displayed as caption on modal footer
